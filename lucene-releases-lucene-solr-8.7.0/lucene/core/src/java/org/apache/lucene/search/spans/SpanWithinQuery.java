...
public final class SpanWithinQuery extends SpanContainQuery {
  ...
  public SpanWeight addFieldNameBeforeCreateWeight(IndexSearcher searcher, ScoreMode scoreMode, float boost) throws IOException {
    SpanWeight bigWeight = big.addFieldNameBeforeCreateWeight(searcher, scoreMode, boost);
    SpanWeight littleWeight = little.addFieldNameBeforeCreateWeight(searcher, scoreMode, boost);
    return new SpanWithinWeight(searcher, scoreMode.needsScores() ? getTermStates(bigWeight, littleWeight) : null,
            bigWeight, littleWeight, boost);
  }
